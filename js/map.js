(function (app) {
	...
	app.closeAllInfoWindows = function () {
		for (let i = 0; i < app.infoWindow.length; i++) {
	...
	app.clearMap = function () {
		...
		for (let i = 0; i < app.poligonoArray1.length; i++) {
	...
	app.resetValues = function () {
		for (let i = 0; i < app.municipi.length; i++) {
	...
	app.loadIncidenti = function () {
		...
		$.ajax({
			...
			success: function (result) {
				...
				app.resetValues();
				const zonaMappa = app.municipi;

				for (let j = 0; j < result.length; j++) { // per ogni mun
					// riportato
			...
			error: function (result) {
				console.log("Error retrieving incidenti in municipi");
				console.log(result);
			},
			complete: function () {
			}
